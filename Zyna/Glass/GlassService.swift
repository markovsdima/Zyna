...
final class GlassService {
    ...
    /// Cached zero-copy capture: MTLBuffer(.shared) backs both CGContext and MTLTexture.
    ...
    /// Zero-copy buffer→texture works on device and Apple Silicon simulator.
    ...
    /// Zero-copy on device (CGContext → MTLBuffer → texture view), fallback on simulator.
    private func captureRegion(_ frame: CGRect, from window: UIWindow, scale: CGFloat,
        ...
        // Zero-copy: GPU reads directly from buffer memory
